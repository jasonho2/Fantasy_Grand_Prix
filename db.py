...
projected_matchups      one row per team for every week AFTER whichever one
                        ...
                        results.
weekly_recaps           one row per (league, season, week) -- a written
                        recap of that week's matchups, generated by an
                        automated weekly task (not this Python pipeline;
                        see web/app/api/recaps/route.js) and upserted via
                        POST /api/recaps, keyed so re-running the same
                        week's recap updates it in place rather than
                        duplicating. Unlike live_matchups/projected_matchups
                        above, this IS a history table -- every past week's
                        recap is kept, never wholesale-replaced -- read by
                        the Contests page (latest week for the active
                        season, shown above the leaderboard) and the
                        dedicated Weekly Report page (every recap for a
                        league, newest first). Gated on leagues.
                        recaps_enabled below -- a league with recaps
                        disabled simply never gets rows written here.
"""
...
CREATE TABLE IF NOT EXISTS leagues (
    ...
    recaps_enabled INTEGER NOT NULL DEFAULT 0,  -- opt-in for the automated weekly recap (see the
                                          -- weekly_recaps table below) -- unlike cup_weeks/scoring_config,
                                          -- this is deliberately league-wide, not per-season: it's a
                                          -- "does this league want this feature at all" toggle, not
                                          -- something whose correct value legitimately varies year to
                                          -- year. Settable at registration time (Add League) or later via
                                          -- Manage Leagues (passphrase-gated PATCH, same as scoring edits,
                                          -- since flipping it triggers an automated task to start posting
                                          -- AI-written content about this league). Defaults off so
                                          -- registering a league never opts it into the feature silently.
...
-- One row per (league, season, week) -- a written recap of that week's
-- matchups (see the docstring's weekly_recaps entry above). Unlike
-- live_matchups/projected_matchups above, this is a real history table:
-- rows accumulate, never wholesale-replaced. Written by POST /api/recaps
-- (an automated weekly task), not by this Python pipeline -- there's no
-- loader function for it here, since the web app upserts directly via its
-- own SQL, the same way it already writes leagues/league_seasons rows for
-- self-service registration and Manage Leagues edits.
...
CREATE INDEX IF NOT EXISTS idx_weekly_recaps_league_season ON weekly_recaps(league_id, season);
...
COLUMN_MIGRATIONS = [
    ...
    # Opt-in flag for the automated weekly recap (see the SCHEMA_SQL comment
    # on leagues.recaps_enabled and the weekly_recaps table). SQLite's ADD
    # COLUMN can't backfill the same "NOT NULL DEFAULT 0" SCHEMA_SQL declares
    # for a brand-new database, so an old database migrated via this path
    # gets NULL here instead of 0 for every existing row -- every read site
    # (api/leagues/route.js's GET, the recap-posting task's league filter)
    # treats NULL the same as 0/false, so this is harmless.
    ("leagues", "recaps_enabled", "INTEGER"),
]
